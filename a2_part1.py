...
def load_game_tree(games_file: str) -> a2_game_tree.GameTree:
    ...
    with open(games_file) as csv_file:
        ...
        new_tree = a2_game_tree.GameTree()
        for row in reader:
            new_tree.insert_move_sequence(row)
    return new_tree
...
class RandomTreePlayer(a2_minichess.Player):
    ...
    def make_move(self, game: a2_minichess.MinichessGame, previous_move: Optional[str]) -> str:
        ...
        if self._game_tree is None or self._game_tree.get_subtrees() == []:
            ...
            choice = random.choice(self._game_tree.get_subtrees())
            self._game_tree = choice
            pick = choice.move
        return pick
...
def part1_runner(games_file: str, n: int, black_random: bool) -> None:
    ...
    if black_random is True:
        a2_minichess.run_games(n, RandomTreePlayer(game_tree), a2_minichess.RandomPlayer())
    else:
        a2_minichess.run_games(n, RandomTreePlayer(game_tree), RandomTreePlayer(game_tree))
