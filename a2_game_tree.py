...
class GameTree:
    ...
    Instance Attributes:
      ...
      - white_win_probability: the probability that White
                               will win from the current state of the game
    ...
    def insert_move_sequence(self, moves: list[str], white_win_probability: float = 0.0) -> None:
        ...
        return self.insert_move1(new_list, white_move=False, white_win_prob=white_win_probability)

    def insert_move1(self, lst: list[str], white_move: bool = False,
                     white_win_prob: float = 0.0) -> None:
        """
        Insert the list into the game tree using the ii) method
        """
        if lst == []:
            return
            ...
            curr = lst.pop()
            result = self.find_subtree_by_move(curr)
            if result is not None:
                new_white_move = not white_move
                return result.insert_move1(lst, new_white_move, white_win_prob)

            else:
                new_tree = GameTree(curr, white_move, white_win_prob)
                self.add_subtree(new_tree)
                new_white_move = not white_move
                return new_tree.insert_move1(lst, new_white_move, white_win_prob)  # check
    ...
    def _update_white_win_probability(self) -> None:
        ...
        if self.get_subtrees() == []:
            return
        elif self.is_white_move is True:
            self.white_win_probability = max(subtree.white_win_probability
                                             for subtree in self.get_subtrees())
        else:
            subtrees_prob = [subtree.white_win_probability for subtree in self.get_subtrees()]
            self.white_win_probability = sum(subtrees_prob) / len(subtrees_prob)
