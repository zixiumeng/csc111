...
def create_book_graph(review_graph: WeightedGraph,
    ...
    """
    graph = WeightedGraph()
    book = review_graph.get_all_vertices("book")
    for item in book:
        graph.add_vertex(item, "book")
        for item2 in book:
            if item != item2:
                score = review_graph.get_similarity_score(item, item2, score_type)
                if score > threshold:
                    graph.add_vertex(item2, "book")
                    graph.add_edge(item, item2, score)
    return graph
...
def cross_cluster_weight(book_graph: WeightedGraph, cluster1: set, cluster2: set) -> float:
    ...
    for item1 in cluster1:
        for item2 in cluster2:
            acc.append(book_graph.get_weight(item1, item2))

    return sum(acc) / (len(cluster2) * len(cluster1))
