...
class MoveToFrontLinkedList(LinkedList):
    ...
    def __contains__(self, item: Any) -> bool:
        ...
        if curr is None:
            ...
            self._first, curr.next, prev.next = curr, self._first, curr.next
            return True
...
class CountLinkedList(LinkedList):
    ...
    def __contains__(self, item: Any) -> bool:
        ...
        curr = self._first
        prev = None
        less_count = self._first
        while not (curr is None or curr.item == item):
            curr, prev = curr.next, curr
            if curr is None:
                return False
            elif curr.access_count != prev.access_count:
                less_count = prev
        if curr is None:
            return False
        elif prev is None:
            ...
            return True
        elif less_count.access_count >= curr.access_count + 1:
            if prev == less_count:
                curr.access_count += 1
            else:
                less_count.next, curr.access_count, prev.next, curr.next =\
                    curr, curr.access_count + 1, curr.next, less_count.next
            return True
        elif less_count.access_count == curr.access_count:
            self._first, curr.next, prev.next, curr.access_count \
                = curr, self._first, curr.next, curr.access_count + 1
            return True
        else:
            less_count.next, curr.next, prev.next, curr.access_count = \
                curr, less_count.next, curr.next, curr.access_count + 1
            return True
