        ...
        if self.degree() == 0 or other.degree() == 0:
            ...
            for ver in self.neighbours:
                if ver in other.neighbours:
                    acc.add(ver)
            num = len(acc)
            deno = len(self.neighbours.union(other.neighbours))
            return num / deno
...
class Graph:
    ...
    def get_similarity_score(self, item1: Any, item2: Any) -> float:
        ...
        return vertex1.similarity_score(vertex2)
    ...
    def recommend_books(self, book: str, limit: int) -> list[str]:
        ...
        *descending order* of similarity score. Ties are broken in descending order
        ...
        for item in self.get_all_vertices("book"):
            if item != book:
                score = self.get_similarity_score(item, book)
...
def load_review_graph(reviews_file: str, book_names_file: str) -> Graph:
    ...
    with open(reviews_file) as review:
        ...
        for row in reader:
            ...
            graph.add_edge(row[0], book[row[1]])
    return graph
